...
int main(int argc,char** argv) {
    try {
        ...
        const U limit=std::stoull(argv[2]);
        const unsigned workers=argc>4?std::stoul(argv[4]):3;
        const U block=argc>5?std::stoull(argv[5]):8388608;
        if(!limit || !workers || !block) throw std::runtime_error("positive parameters required");
        std::ifstream in(argv[1]);
        if(!in) throw std::runtime_error("missing term list");
        std::string line;
        std::getline(in,line);
        std::vector<U> a;
        while(std::getline(in,line)) a.push_back(std::stoull(line.substr(line.find(',')+1)));
        ...
        for(unsigned w=0;w<workers;++w) threads.emplace_back([&,w] {
            try {
                std::vector<uint32_t> hist(block);
